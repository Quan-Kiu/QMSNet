...
const SavedItem = ({ post }) => {
    ...
    const preViewPost = post.media?.length > 0 ? post?.media[0]?.url.includes('/image/') && post?.media[0]?.url : post?.user?.avatar?.url

    return (
        <SavedItemWrapper>
            <Row gutter={[32, 32]} align="middle">
                <Col xl={4} md={4} sm={6} sx={8}>
                    <img className="main-image" src={preViewPost} alt="logo" />
                </Col>
